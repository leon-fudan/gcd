...
set power_cg_auto_identify true ;# Set before RTL is read if used
...
report_guidance -to ${REPORTS_DIR}/ceva.mapped.svf.rpt
...
read_verilog -r -vcs "-f ${RTL_SOURCE_FILES}" -work_library WORK
...
set_top r:/WORK/${DESIGN_NAME}_${DESIGN_NAME}

#################################################################################
# For a UPF MV flow, read in the UPF file for the Reference Design
#################################################################################

load_upf -r "./inputs/${DCRM_MV_UPF_INPUT_FILE}"
...
load_upf -i "./inputs/${DCRM_MV_UPF_INPUT_FILE}"
